 ...
 * @param pos 
 */
function isValidDelimiter(state: StateCore, pos: number) {
...
function plugin(md: MarkdownIt, options: Options) {
  ...
  options = { blockWrapper: 'div', ...options };
   ...
   */
  const renderInline = (tokens: Token[], idx: number, opts: any, env: any) => {
    const maths = tokens[idx].content;
    const throwOnError = typeof options.throwOnError === 'function'
      ? options.throwOnError(env)
      : !!options.throwOnError;
    try {
      const rendered = katex.renderToString(maths, { ...options, throwOnError, displayMode: false });
      return `${rendered}\n`;
    } catch (error) {
      if (throwOnError) {
        error.diagnostics = { ...error.diagnostics, source: maths };
        throw error;
      }
   ...
   * @param idx Index of the current token.
   */
  const renderBlock = (tokens: Token[], idx: number, opts: any, env: any) => {
    const maths = tokens[idx].content;
    const throwOnError = typeof options.throwOnError === 'function'
      ? options.throwOnError(env)
      : !!options.throwOnError;
    try {
      const rendered = katex.renderToString(maths, { ...options, throwOnError, displayMode: true });
